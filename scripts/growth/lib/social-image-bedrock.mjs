...
export const DEFAULT_NEGATIVE_PROMPT = [
  'minors', 'teenagers', 'children', 'nudity', 'explicit sexual activity',
  'distorted anatomy', 'malformed hands', 'extra fingers', 'extra arms', 'duplicated people',
  'distorted faces', 'plastic skin', 'cartoon', 'illustration', 'anime', 'fake text',
  'watermark', 'random logo', 'stock photo handshake pose', 'empty gradient background',
  'text card design', 'corporate clipart', 'blurry faces', 'deformed limbs', 'uncanny valley',
  'lonely isolated person', 'generic running silhouette', 'anonymous distant hikers',
  'people too small in frame', 'faces hidden in shadow', 'cold impersonal stock photography',
  'laptops', 'computer screens', 'office desks', 'office meetings', 'coworking space',
  'business meetings', 'conference rooms', 'people working on laptops',
  'people studying with textbooks', 'corporate networking', 'business suits'
].join(', ');
...
export function buildPhotographyPrompt(concept) {
  ...
  const activity = concept?.photoPrompt || concept?.visualConcept ||
    (mode === 'VIBE'
      ? 'attractive adult friends sharing an active social experience at golden hour, laughing together after an activity'
      : mode === 'DATE'
        ? 'fit attractive adult couple sharing a playful active date, warm chemistry and genuine mutual attraction'
        : 'two fit adult workout partners training side by side and encouraging each other');

  if (mode === 'VIBE') {
    return (
      'Premium photorealistic commercial lifestyle campaign. ' + BRAND_CORE +
      `Scene: ${activity}. ` +
      'Show 2–4 clearly identifiable adults approximately 25–45 years old as large hero subjects, interacting with each other. ' +
      ...
      'Cinematic golden-hour or premium venue lighting, dynamic composition, shallow depth of field. ' +
      'Do not make this look like anonymous tourism or generic hikers; faces and connection must be obvious. ' +
  ...
  return (
    'Premium photorealistic commercial sports-lifestyle campaign. ' + BRAND_CORE +
    `Scene: ${activity}. ` +
    ...
    'Modern premium sportswear, natural athletic physiques, realistic skin texture and anatomy, professional sports photography. ' +
    'Cinematic lighting, dynamic action, sophisticated gym or outdoor fitness environment, shallow depth of field. ' +
    'Never show a lone anonymous athlete or generic running silhouettes; the social connection must be obvious. ' +
    'Leave clean natural negative space near the bottom for a short branded headline/CTA overlay. No text, no logo, no watermark.'
  );
}

function writeTempJson(obj) {
  const file = path.join(os.tmpdir(), `gtm-stable-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
  fs.writeFileSync(file, JSON.stringify(obj));
  return file;
}

export function invokeStableImageCore({ prompt, negativePrompt = DEFAULT_NEGATIVE_PROMPT, aspectRatio = '4:5', seed = 0, outputFormat = 'jpeg', region = BEDROCK_IMAGE_REGION, modelId = STABLE_IMAGE_CORE_MODEL_ID } = {}) {
  const body = { prompt, negative_prompt: negativePrompt, aspect_ratio: aspectRatio, seed, output_format: outputFormat };
  const reqFile = writeTempJson(body);
  const outFile = path.join(os.tmpdir(), `gtm-stable-out-${Date.now()}.json`);
  try {
    const r = spawnSync('aws', ['bedrock-runtime','invoke-model','--model-id',modelId,'--content-type','application/json','--accept','application/json','--body',`fileb://${reqFile}`,'--cli-binary-format','raw-in-base64-out',outFile,'--region',region], { encoding: 'utf8', stdio: 'pipe', maxBuffer: 20 * 1024 * 1024 });
    if (r.status !== 0) return { ok: false, error: (r.stderr || r.stdout || 'bedrock invoke failed').slice(0, 500), modelId };
    const parsed = JSON.parse(fs.readFileSync(outFile, 'utf8'));
    if (parsed.error) return { ok: false, error: String(parsed.error), modelId };
    const finishReason = parsed.finish_reasons?.[0];
    if (finishReason && finishReason !== 'SUCCESS') return { ok: false, error: `stable_image_${finishReason.toLowerCase()}`, modelId };
    const b64 = parsed.images?.[0];
    if (!b64) return { ok: false, error: 'stable_image_no_images', modelId };
    return { ok: true, buffer: Buffer.from(b64, 'base64'), modelId, seed: parsed.seeds?.[0] ?? seed, aspectRatio };
  } finally {
    for (const f of [reqFile, outFile]) { try { fs.unlinkSync(f); } catch { /* ignore */ } }
  }
}

export async function assessPhotoQuality(buffer, sharpImpl) {
  const sharp = sharpImpl || (await import('sharp')).default;
  if (!buffer || buffer.length < 40_000) return { ok: false, reason: 'file_too_small', bytes: buffer?.length || 0 };
  const img = sharp(buffer);
  const meta = await img.metadata();
  if (!meta.width || !meta.height || meta.width < 512 || meta.height < 512) return { ok: false, reason: 'resolution_too_low', meta };
  const stats = await img.stats();
  const channels = stats.channels || [];
  const avgStdev = channels.reduce((sum, c) => sum + (c.stdev || 0), 0) / Math.max(channels.length, 1);
  const avgMean = channels.reduce((sum, c) => sum + (c.mean || 0), 0) / Math.max(channels.length, 1);
...
export async function generateBedrockPhoto(concept, { seed, sharpImpl, maxAttempts = 2 } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const invoked = invokeStableImageCore({ prompt, seed: attemptSeed % 4_294_967_295, aspectRatio: '4:5', outputFormat: 'jpeg' });
    if (!invoked.ok) { lastError = invoked.error; continue; }
    const quality = await assessPhotoQuality(invoked.buffer, sharpImpl);
    if (!quality.ok) { lastError = quality.reason; continue; }
    return { ok: true, buffer: invoked.buffer, modelId: invoked.modelId, seed: attemptSeed, prompt, quality };
  }
