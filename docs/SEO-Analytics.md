...
| `VITE_GA_MEASUREMENT_ID` | GA4 Measurement ID. **Production builds** default to `G-C29M8NWNY4` if unset. In local dev, gtag stays off unless you set this in `.env`. |
...
1. Deploy the production web build (measurement ID `G-C29M8NWNY4` is baked in unless you override `VITE_GA_MEASUREMENT_ID`).
